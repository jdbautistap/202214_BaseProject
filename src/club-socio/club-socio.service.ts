...
export class ClubSocioService {
    ...
    async addSocioCLub(clubId: string, socioId: string): Promise<ClubEntity> {
        ...
        if (!socio)
          throw new BusinessLogicException("El socio con el id no fue encontrado", BusinessError.NOT_FOUND);
      
        const club: ClubEntity = await this.clubRepository.findOne({where: {id: clubId}, relations: ["socios"]})
        if (!club)
          throw new BusinessLogicException("El club con el id no fue encontrado", BusinessError.NOT_FOUND);
    ...
    async associateSociosClub(clubId: string, socios: SocioEntity[]): Promise<ClubEntity> {
        ...
        for (let i = 0; i < socios.length; i++) {
          ...
          if (!socio)
            throw new BusinessLogicException("El socio con el id no se encontró", BusinessError.NOT_FOUND)
        }
